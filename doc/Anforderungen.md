...
Bedienung über Clients, die nur mit dem Backend sprechen. Web-GUI für Bestand und Verwaltung; Flutter-App (`mobile/`) für Direktaufnahme (Kamera + GPS). Später möglich: Desktop (z. B. Qt).
...
- Bild hochladen; Arbeitsoriginal speichern (längste Seite höchstens 2048 px, sonst verkleinern); Thumbnails ablegen.
...
- Optional: Erkennungsrechteck am Foto (vom Bearbeiter aufgezogen). CLIP nutzt nur diesen Ausschnitt; das gespeicherte Bild bleibt vollständig.
...
## Mobile-Aufnahme (Flutter)

Ziel: vor Ort fotografieren, ohne auf EXIF in Galerie-Uploads angewiesen zu sein.

### Funktional

- Anmeldung wie Web (Cookie-Sitzung `sf_session`); API-Token bleiben nur lesend.
- Foto mit Geräte-Kamera; GPS zum Aufnahmezeitpunkt (Genauigkeit ± m); optionale Notiz, Tags und Vorgang; Upload an `POST /api/records`.
- Ohne GPS oder bei schlechtem Fix: Ort auf der Karte setzen; sonst später in der Web-UI.
- Serienaufnahme (Kamera-Wiederholung oder Mehrfachauswahl Galerie); Queue drainiert bei Netz-Wiederkehr.
- Deep-Link „Im Web öffnen“ zu `/datensatz/{id}` — keine zweite Voll-UI (Liste, Motive, Admin nur im Web).

### Technisch

- Flutter unter `mobile/`, Clients nur HTTP-API.
- Android first; iOS-Zielplattform mitvorbereitet.
...
- Mehrere parallele Voll-UIs (Web + Qt + vollständige Mobile-App). Die Flutter-App bleibt auf Direktaufnahme beschränkt.
