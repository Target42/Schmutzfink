...
type ListFilter struct {
	...
	To           *time.Time
	HasGPS       *bool
	Edited       *bool // false = nur Ganzbild ohne Motiv; true = ROI, Motiv oder mehrere Sichtungen
	NearLat      *float64
	NearLon      *float64
	RadiusM      *float64
	SkipText     bool
...
func (r *Repo) List(ctx context.Context, tenantID string, f ListFilter) ([]Record, int, error) {
	...
	var total int
	// Bestand: one tile per photo, even if several sightings match the filter.
	countSQL := `SELECT count(DISTINCT r.id) FROM sightings s JOIN records r ON r.id = s.record_id ` + where
	...
	SELECT * FROM (
		SELECT DISTINCT ON (r.id)
			s.id, r.id AS record_id, s.note, s.tags, r.lat, r.lon, r.captured_at, r.uploaded_at,
	...
	ORDER BY COALESCE(hits.captured_at, hits.uploaded_at) DESC, hits.uploaded_at DESC, hits.id
...
func (r *Repo) UpdatePhoto(ctx context.Context, tenantID, id string, photo Photo) (Photo, error) {
	...
	if photo.Extra != nil {
		...
		extra = b
	} else if cur.Extra != nil {
		b, err := json.Marshal(cur.Extra)
		if err != nil {
			return Photo{}, err
		}
		extra = b
	...
	_, err = r.DB.Exec(ctx, `
		...
		tenantID, id, photo.Lat, photo.Lon, photo.CapturedAt, extra)
	if err != nil {
		return Photo{}, err
	}
...
const photoSQL = `
	SELECT r.id, r.lat, r.lon, r.captured_at, r.uploaded_at,
	       ...
	       r.location_redacted_at, c.closed_at, COALESCE(r.extra, '{}'::jsonb)
	FROM records r
	JOIN users u ON u.id = r.uploaded_by
...
func buildWhere(tenantID string, f ListFilter) (string, []any) {
	...
	if f.Edited != nil {
		// Offen: genau eine Ganzbild-Sichtung ohne Motiv. Bearbeitet: ROI, Motiv oder >1 Sichtung.
		const catalogued = `(EXISTS (
			SELECT 1 FROM sightings o
			WHERE o.record_id = r.id AND (o.roi_x IS NOT NULL OR o.motif_id IS NOT NULL)
		) OR (SELECT count(*) FROM sightings c WHERE c.record_id = r.id) > 1)`
		if *f.Edited {
			parts = append(parts, `AND `+catalogued)
		} else {
			parts = append(parts, `AND NOT `+catalogued)
		}
	}
...
func scanPhoto(row scanner) (Photo, error) {
	...
	if len(extra) > 0 {
		_ = json.Unmarshal(extra, &p.Extra)
	}
