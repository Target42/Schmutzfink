...
func (r *Repo) SearchByVector(ctx context.Context, tenantID string, vec []float32, f ListFilter, excludeID string) ([]Record, int, error) {
	...
	if excludeID != "" {
		// Drop the whole photo of the excluded sighting, not only that ROI.
		args = append(args, excludeID)
		where += ` AND s.record_id <> (SELECT record_id FROM sightings WHERE id = $` + itoa(len(args)) + `)`
	}
	...
	var total int
	if err := r.DB.QueryRow(ctx, `SELECT count(DISTINCT r.id) FROM sightings s JOIN records r ON r.id = s.record_id `+where, args...).Scan(&total); err != nil {
		return nil, 0, err
	}

	// One hit per photo: keep the closest matching sighting.
	listSQL := `
	SELECT * FROM (
		...
		` + where + `
		ORDER BY r.id, s.embedding <=> $` + vecPH + `
	) AS hits
	ORDER BY hits.score DESC NULLS LAST, hits.id
