import { defaultMinScore, listPageSize, type APIToken, type AuditEvent, type Case, type CaseKind, type CustomField, type Filters, type GeoAddress, type ImportResult, type Motif, type PhotoDetail, type RecordItem, type Summary, type User } from './types.ts'
...
function filterParams(filters: Filters) {
  ...
  if (filters.edited) q.set('edited', filters.edited)
...
export const api = {
  ...
  mobileAndroid: () => request<AndroidMobileInfo>('/api/mobile/android'),
  ...
  geocode: (q: string) =>
    request<{ lat: number; lon: number; label: string; address: GeoAddress }>(
