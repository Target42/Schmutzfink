...
const deletedPopupHtml = '<p class="map-popup-gone">Sichtung bereits gelöscht</p>'
...
export function GraffitiMap({
  ...
  const onMissingRef = useRef(onMissingPoint)
  ...
  onMissingRef.current = onMissingPoint
    ...
    for (const p of points) {
      ...
      if (popups) {
        ...
        bindPopupGuards(m, p, {
          ...
          onMissing: (id) => onMissingRef.current?.(id),
        })
      }
...
function popupHtml(p: MapPoint) {
  const img = p.thumb_url ? `<p><img src="${escapeHtml(p.thumb_url)}" alt="" width="160" /></p>` : ''
  const label = escapeHtml(p.note || 'Sichtung')
  if (!p.recordId) {
    return `${img}<span>${label}</span>`
  }
  const href = `/datensatz/${p.recordId}?s=${p.id}`
  return `${img}<a href="${href}" class="map-sighting-link">${label}</a>`
}

function bindPopupGuards(
  marker: L.Marker,
  point: MapPoint,
  hooks: { navigate: (path: string) => void; onMissing: (id: string) => void },
) {
  if (!point.recordId) return
  const recordId = point.recordId
  let checking = false
  ...
  function markGone() {
    if (gone) return
    gone = true
    marker.setPopupContent(deletedPopupHtml)
    marker.unbindTooltip()
    const remove = () => hooks.onMissing(point.id)
    marker.once('popupclose', remove)
    window.setTimeout(remove, 2500)
  }

  async function ensureExists(): Promise<boolean> {
  ...
  marker.on('popupopen', () => {
    ...
    const link = el.querySelector<HTMLAnchorElement>('a.map-sighting-link')
    if (!link) return
    link.addEventListener('click', (ev) => {
      ...
      void (async () => {
        ...
        if (!ok) return
        hooks.navigate(`/datensatz/${recordId}?s=${point.id}`)
      })()
    })
  })
}
...
/** Friendly detail-page message when a map link targets a deleted record. */
export function missingSightingMessage(err: unknown) {
  if (err instanceof ApiError && err.status === 404) return 'Sichtung bereits gelöscht'
