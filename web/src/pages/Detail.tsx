...
import { GraffitiMap, type MapPoint } from './MiniMap.tsx'
...
export function DetailPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err: unknown) => {
        if (!stopped) setError(err instanceof Error ? err.message : 'Fehler')
      })
    return () => {
      stopped = true
    }
  ...
  return (
    <section className={editing ? 'detail workspace' : 'detail detail-view workspace'}>
      ...
      <Split id="sf.split.detail.cols" axis="x" defaultPct={56} minA={260} minB={260}>
        {photoStage}
          <form className="panel stack form-scroll" onSubmit={(e) => void onSubmit(e)}>
            <SightingTabs
              photo={photo}
            ...
            {selected?.motif_id ? (
              ...
              <button type="button" disabled={motifBusy !== ''} onClick={() => void startMotif()}>
                Motiv beginnen
              </button>
            )}
      ...
      <Split id="sf.split.detail.cols" axis="x" defaultPct={56} minA={260} minB={260}>
        ...
        <Split id="sf.split.detail.side" axis="y" defaultPct={54} minA={140} minB={160}>
            ...
            <div className="motif-actions">
              {selected?.motif_id ? (
                <>
                  ...
                  {writable ? (
                    <button type="button" className="linkish" disabled={motifBusy !== ''} onClick={() => void unlinkMotif()}>
                      Vom Motiv lösen
                    </button>
                  ) : null}
                </>
          ...
          <div className="panel detail-map">
            {mapPoints.length ? (
              <>
                <GraffitiMap className="map map-overview" points={mapPoints} focusId={selected?.id} />
